extends Control
const menu_opciones = preload("res://Juego/sonidos/wavs/menu_opciones.wav")
...
func _ready() -> void:
	VariablesGlobales.playtts(menu_opciones,%TTS)
	await(%TTS.finished)
	#VariablesGlobales.tts_habla("Menú de Opciones",0)
